...
export default async function FoodDetail({ params }: Props) {
  ...
  return (
    <section className="mx-auto max-w-6xl">
      ...
      {/* Main card */}
      <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-white/3 shadow-2xl shadow-pink-500/5 backdrop-blur">
          ...
          <div className="flex flex-col justify-center p-8 md:p-12">
            ...
            <div className="mb-10 grid grid-cols-2 gap-4">
              ...
              <div className="rounded-2xl border border-white/10 bg-white/3 p-4">
                ...
                <p className="mt-1 text-xl font-bold text-indigo-300">{Number(foodItem.price).toFixed(2)} €</p>
              </div>
            </div>

            {/* Favorite */}
            <div className="relative z-10 flex justify-center border-red-200 gap-2">
              <AddFavoritFoodButton foodId={foodItem.id} isDetail={true} />
              <span className="text-red-400">Add to my favorites</span>
            </div>
          </div>
        </div>
      </div>
